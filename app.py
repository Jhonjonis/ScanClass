...
def salvar_rosto():
    ...
    try:
        ...
        return jsonify({"erro": "Imagem inválida ou corrompida"}), 400
...
def reconhecimento_facial():
    ...
    try:
        with conn.cursor() as cur:
            ...
            if not usuarios:
                return jsonify({"erro": "Nenhum usuário com rosto cadastrado"}), 400
    ...
    if usuario_input and matched.lower() != usuario_input.lower():
        return jsonify({"erro": f"Rosto reconhecido como {matched}, mas você informou {usuario_input}."}), 400
...
def login():
    if request.method == 'POST':
        ...
        try:
            with conn.cursor() as cur:
                cur.execute("SELECT * FROM usuarios WHERE usuario=%s OR email=%s", 
